...
import { analyzeRisk, API_BASE_URL, createRazorpayOrder, explainInvestigation, getHealth, getIntegrationStatus, getRecentAudit, type RiskPayload } from "@/lib/riskpilotApi";
  ...
  return <div className="min-h-screen bg-[#f5f5ee] px-4 py-8 text-[#26343b] sm:px-8 lg:px-16"><div className="mx-auto max-w-7xl">
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
      <section className="rp-surface rounded-xl p-5"><div className="mb-5 flex items-center justify-between"><div><p className="rp-eyebrow text-[#748187]">Transaction payload</p><h2 className="mt-1 text-lg font-semibold">Risk decision input</h2></div><span className="rounded-full bg-[#edf8f4] px-3 py-1 text-[11px] font-semibold text-[#0e897d]">{statusLabel}</span></div>
        ...
        <div className="mt-5 flex flex-wrap gap-2"><button disabled={busy} onClick={() => void run("Risk analysis", () => analyzeRisk(payload))} className="rp-button rp-button-ink rounded-lg px-4 py-2.5 text-xs font-semibold disabled:opacity-50">Analyze transaction</button><button disabled={busy} onClick={() => void run("Investigation", () => explainInvestigation(payload))} className="rp-button rounded-lg border border-[#cbd2ce] bg-white px-4 py-2.5 text-xs font-semibold disabled:opacity-50">Explain with AI</button><button disabled={busy} onClick={() => void run("Razorpay order", () => createRazorpayOrder({ ...payload, currency: "INR", notes: { source: "riskpilot-live-lab" } }))} className="rp-button rounded-lg border border-[#cbd2ce] bg-white px-4 py-2.5 text-xs font-semibold disabled:opacity-50">Create test order</button></div>
      </section>
